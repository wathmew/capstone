...
def print_scores(scores):
    ...
    if "trigger_id" in scores:
        print('Tri-I            - P: {:5.2f} ({:4d}/{:4d}), R: {:5.2f} ({:4d}/{:4d}), F: {:5.2f}'.format(
    ...
    if "trigger_cls" in scores:
        print('Tri-C            - P: {:5.2f} ({:4d}/{:4d}), R: {:5.2f} ({:4d}/{:4d}), F: {:5.2f}'.format(
    ...
    if "argument_id" in scores:
        print('Arg-I            - P: {:5.2f} ({:4d}/{:4d}), R: {:5.2f} ({:4d}/{:4d}), F: {:5.2f}'.format(
    ...
    if "argument_cls" in scores:
        print('Arg-C            - P: {:5.2f} ({:4d}/{:4d}), R: {:5.2f} ({:4d}/{:4d}), F: {:5.2f}'.format(
    ...
    if "argument_attached_id" in scores:
        print('Arg-I (attached) - P: {:5.2f} ({:4d}/{:4d}), R: {:5.2f} ({:4d}/{:4d}), F: {:5.2f}'.format(
    ...
    if "argument_attached_cls" in scores:
        print('Arg-C (attached) - P: {:5.2f} ({:4d}/{:4d}), R: {:5.2f} ({:4d}/{:4d}), F: {:5.2f}'.format(
...
def compute_seq_arg_scores(preds, golds):
    assert len(preds) == len(golds)
    scores = {}
    scores["argument_id"] = compute_seq_argument_id_score(preds, golds)
    scores["argument_cls"] = compute_seq_argument_cls_score(preds, golds)
    return scores

def compute_seq_argument_id_score(preds, golds):
    pred_arg_id, gold_arg_id = [], []
    for i, (pred, gold) in enumerate(zip(preds, golds)):
        pred_arg_id.extend([ (i, p[0]) for p in pred ])
        gold_arg_id.extend([ (i, g[0]) for g in gold ])
...
def compute_seq_argument_cls_score(preds, golds):
    pred_arg_cls, gold_arg_cls = [], []
    for i, (pred, gold) in enumerate(zip(preds, golds)):
        pred_arg_cls.extend([ (i, p[0], p[1]) for p in pred ])
        gold_arg_cls.extend([ (i, g[0], g[1]) for g in gold ])
