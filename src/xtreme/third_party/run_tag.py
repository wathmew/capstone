...
from utils_tag import convert_examples_to_features_token, get_labels, read_examples_from_file
...
from model import CustomXLMRobertaForTokenClassification
...
MODEL_CLASSES = {
  ...
  "xlmr": (XLMRobertaConfig, CustomXLMRobertaForTokenClassification, XLMRobertaTokenizer),
}

class TokenDataset(torch.utils.data.Dataset):

  def __init__(self, all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_token_lengths):
    super().__init__()
    self.all_input_ids = all_input_ids
    self.all_input_mask = all_input_mask
    self.all_segment_ids = all_segment_ids
    self.all_label_ids = all_label_ids
    self.all_token_lengths = all_token_lengths
    assert len(all_input_ids) == len(all_input_mask) == len(all_segment_ids) == len(all_label_ids) == len(all_token_lengths)

  def __getitem__(self, idx):
    return tuple((self.all_input_ids[idx], self.all_input_mask[idx], self.all_segment_ids[idx], self.all_label_ids[idx], self.all_token_lengths[idx]))
...
def token_collate_fn(batch):
  ...
  all_input_ids = torch.stack([b[0] for b in batch])
  all_input_mask = torch.stack([b[1] for b in batch])
  all_segment_ids = torch.stack([b[2] for b in batch])
  all_label_ids = torch.stack([b[3] for b in batch])
  all_token_lengths = [b[4] for b in batch]
  return all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_token_lengths
...
def train_token(args, train_dataset, model, tokenizer, labels, pad_token_label_id, lang2id=None):
  """Train the model."""
  ...
  train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, collate_fn=token_collate_fn)
  ...
  for _ in train_iterator:
    ...
    for step, batch in enumerate(epoch_iterator):
      ...
      inputs = {"input_ids": batch[0].to(args.device),
            ...
            "token_lengths": batch[4],
            "multi_piece": args.multi_piece}
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix="", lang="en", lang2id=None, print_result=True):
  ...
  eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, collate_fn=token_collate_fn)
  ...
  for batch in tqdm(eval_dataloader, desc="Evaluating"):
    ...
    if preds is None:
      # import IPython.core; IPython.core.debugger.Pdb().set_trace()
      pad_length = inputs["labels"].size()[1] - logits.size()[1]
      preds = logits.detach().cpu().numpy()
      preds = np.pad(preds, [(0, 0), (0, pad_length), (0,0)], mode='constant')
      out_label_ids = inputs["labels"].detach().cpu().numpy()
    else:
      pad_length = inputs["labels"].size()[1] - logits.size()[1]
      new_preds = np.pad(logits.detach().cpu().numpy(), [(0, 0), (0, pad_length), (0,0)], mode='constant')
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode, lang, lang2id=None, few_shot=-1, tt_files=[], tt_langs=[]):
  ...
  cached_features_file = os.path.join(args.data_dir, "cached_token_{}_{}_{}_{}".format(mode, lang,
  ...
  if os.path.exists(cached_features_file) and not args.overwrite_cache:
    ...
    for lg in langs:
      ...
      features_lg = convert_examples_to_features_token(examples, labels, args.max_seq_length, tokenizer,
  ...
  # import IPython.core; IPython.core.debugger.Pdb().set_trace()
  if len(tt_files) > 0:
    assert len(tt_files) == len(tt_langs)
    for tt_file, tt_lang in zip(tt_files, tt_langs):
      logger.info("Creating features from dataset file at {} in language {}".format(tt_file, tt_lang))
      examples = read_examples_from_file(tt_file, tt_lang, lang2id)
      features_tt_lang = convert_examples_to_features_token(examples, labels, args.max_seq_length, tokenizer,
                          cls_token_at_end=bool(args.model_type in ["xlnet"]),
                          cls_token=tokenizer.cls_token,
                          cls_token_segment_id=2 if args.model_type in ["xlnet"] else 0,
                          sep_token=tokenizer.sep_token,
      ...
      features.extend(features_tt_lang)
  ...
  all_token_lengths = [f.token_lengths for f in features]
  ...
  if args.model_type == 'xlm' and features[0].langs is not None:
    ...
    dataset = TokenLangDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_langs, all_token_lengths)
  else:
    dataset = TokenDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_token_lengths)
  return dataset
...
def main():
  ...
  parser.add_argument("--multi_piece", type=str, default="average", help="how to collate subwords")
  ...
  parser.add_argument("--tt_files", default=[], nargs='*', help="filepath for translate-train files")
  parser.add_argument("--tt_langs", default=[], nargs='*', help="translate-train languages corresponding to the tt_files")
  ...
  # Training
  if args.do_train:
    train_dataset = load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode="train", lang=args.train_langs, lang2id=lang2id, few_shot=args.few_shot, tt_files=args.tt_files, tt_langs=args.tt_langs)
    global_step, tr_loss = train_token(args, train_dataset, model, tokenizer, labels, pad_token_label_id, lang2id)
