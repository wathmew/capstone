...
class InputFeaturesToken(object):
  """A single set of features of data."""

  def __init__(self, input_ids, input_mask, segment_ids, token_lengths, label_ids, langs=None):
    self.input_ids = input_ids
    self.input_mask = input_mask
    self.segment_ids = segment_ids
  ...
  with open(file_path, encoding="utf-8") as f:
    ...
    if words:
      ...
      examples.append(InputExample(guid="%s-%d".format(lang, guid_index),
                     ...
                     labels=labels,
                     langs=langs))
  print ("Read %d examples from %s file with max length: %d" % (len(examples), file_path, max_length))
  return examples
...
def convert_examples_to_features_token(examples,
                 label_list,
                 max_seq_length,
                 tokenizer,
                 cls_token_at_end=False,
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    for word, label in zip(example.words, example.labels):
      ...
      if len(word) != 0 and len(word_tokens) == 0:
        word_tokens = [tokenizer.unk_token]
      if len(tokens) + len(word_tokens) > max_seq_length - special_tokens_count:
        print('truncate token', len(tokens), max_seq_length, special_tokens_count)
        break
      tokens.extend(word_tokens)
      token_lengths.append(len(word_tokens))
      # Use the real label id for the first token of the word, and padding ids for the remaining tokens
    ...
    assert len(segment_ids) == max_seq_length
    assert len(label_ids) == max_seq_length
    assert sum(input_mask) - special_tokens_count == sum(token_lengths)
    assert len(langs) == max_seq_length
    ...
    if ex_index < 5:
      ...
      logger.info("input_mask: %s", " ".join([str(x) for x in input_mask]))
      logger.info("segment_ids: %s", " ".join([str(x) for x in segment_ids]))
      logger.info("token_lengths: %s", " ".join([str(x) for x in token_lengths]))
      logger.info("label_ids: %s", " ".join([str(x) for x in label_ids]))
      logger.info("langs: {}".format(langs))

    features.append(
        InputFeaturesToken(input_ids=input_ids,
                input_mask=input_mask,
                segment_ids=segment_ids,
